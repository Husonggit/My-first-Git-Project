...
import win32com.client    #引用win32的库文件
...
class easyExcel:    
      def __init__(self, filename=None):  #打开文件或者新建文件（如果不存在的话）  
          ...
          if filename:    
              ...
              self.xlBook = self.xlApp.Workbooks.Open(filename)    #打开名字为filename的Excel表格
          else:    
              self.xlBook = self.xlApp.Workbooks.Add()    #新建Excel表格
              self.filename = ''  
        
      def save(self, newfilename=None):  #保存文件  
          if newfilename:    
              self.filename = newfilename    
              self.xlBook.SaveAs(newfilename)    #保存新文件
          else:    
              self.xlBook.Save()       
      ...
      def inserRow(self,sheet,row): #插入新的工作表
          sht = self.xlBook.Worksheets(sheet)
